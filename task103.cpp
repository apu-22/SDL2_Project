...
void update() {
    ...
    if (col) {
        X = -R;
        if (f == 0) {
            r_color = 0; b_color = 255; f = 1;
        } else {
            r_color = 255; b_color = 0; f = 0;
        }
    } else {
        X += 2;
...
void Draw() {
    SDL_SetRenderDrawColor(rend, 100, 155, 0, 255); // Background color
    SDL_RenderClear(rend);

    update();

    SDL_SetRenderDrawColor(rend, r_color, 0, b_color, 255); // Color for first circle
    draw_Circle(X, Y, R);

    SDL_SetRenderDrawColor(rend, b_color, 0, r_color, 255); // Color for second circle
    draw_Circle(x, y, R);
    ...
    while (SDL_PollEvent(&event)) {
        if (event.type == SDL_QUIT) {
            ...
            if (event.key.keysym.sym == SDLK_UP) {
                y -= 10;
                if (y - R <= 0) { x = SCREEN_WIDTH / 2; y = SCREEN_HEIGHT - R; }
            }
            ...
            if (event.key.keysym.sym == SDLK_LEFT) {
                x -= 10;
                if (x - R <= 0) { x = SCREEN_WIDTH / 2; y = SCREEN_HEIGHT - R; }
            }
